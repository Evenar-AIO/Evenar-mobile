...
import {
  ...
  ActivityIndicator, Platform, Image, KeyboardAvoidingView, Modal, Alert,
} from 'react-native';
...
export default function CreateEventScreen() {
  ...
  const [tickets, setTickets] = useState<TicketInfoPayload[]>([
    { type: 'Thường', price: '0', quantity: '100', description: '' }
  ]);

  const [values, setValues] = useState<EventPayload>({
    name: '', description: '', startTime: '', endTime: '',
    physicalLocation: '', layout: '', imageURL: '', genreId: '', totalTicketCount: '100',
    ticketInfo: []
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') window.alert('Tạo sự kiện thành công!');
      router.replace('/owner/event');
    } catch (err: any) {
      const msg = err.message || 'Tạo sự kiện thất bại!';
      Platform.OS === 'web' ? window.alert(msg) : Alert.alert('Lỗi', msg);
    } finally {
      setLoading(false);
    }
          ...
          <Pressable onPress={pickImage} style={s.imagePicker}>
            ...
            {uploading && (
              <View style={s.uploadOverlay}>
                ...
                <Text style={s.uploadingTxt}>Đang tải lên...</Text>
              </View>
            )}
          </Pressable>
...
const s = StyleSheet.create({
  ...
  uploadingTxt: { fontSize: 12, fontWeight: '500', color: '#94A3B8', marginTop: 8 },
  ...
  // Submit
  submitBtn: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8, marginTop: 12, borderRadius: 16, paddingVertical: 18, backgroundColor: '#10B981' },
